...
document.addEventListener("click", async (event) => {
  ...
  if (slotCell && !slotCell.disabled) {
    ...
    const stateLabel = slotCell.querySelector("span");
    if (stateLabel)
  ...
  if (register) {
    ...
    try {
      await api(`/api/events/${button.dataset.id}/register`, {
        method: "POST",
      });
  ...
  if (pay) {
    ...
    if (select) {
      select.value =
        state.registrations.find((item) => item.id === Number(pay.dataset.id))
          ?.id || select.value;
